...
- Local ALTCHA challenges are signed with an unambiguous expiry format and can be accepted only once.
...
- Forgot-password captcha protects only the reset-link request; the authenticated new-password confirmation form is never blocked by captcha.
...
ALTCHA local challenges are generated by the module through the `altchachallenge` front controller and signed with the configured HMAC secret. Each successfully accepted local challenge is recorded until it expires and cannot be reused. ALTCHA Sentinel uses the configured Sentinel challenge URL and verifies the solved payload server-side.

After updating the module, use the PrestaShop module upgrade process rather than copying files alone, so the database migration is applied. The local ALTCHA replay registry also self-initializes safely when it is missing. Rotate the ALTCHA HMAC secret after applying a security update to invalidate any previously issued challenge signatures.
...
The front-office script injects captcha widgets into visible supported forms and listens to checkout refresh events so the widget is reattached when PrestaShop updates checkout steps through AJAX. When a theme places submit buttons inside flex or action containers, the widget is inserted before that action row so the captcha keeps its own full-width line. Server-side validation remains the source of truth: a form submission is rejected when the configured captcha, email, or message rule fails, regardless of client-side behavior.
